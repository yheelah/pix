...
imap_username = input("Masukkan Email Hotmail/Outlook: ")
imap_password = input("Masukkan Password: ")
...
def request_otp(email):
    try_count = 0
    while try_count < 3:  # Mencoba maksimal 3 kali
        response = requests.post('https://api.pixelverse.xyz/api/otp/request', json={'email': email})
        if response.status_code == 200:
            return True
        else:
            print(Fore.RED + Style.BRIGHT + f"Permintaan OTP gagal. Status: {response.status_code}, Respon: {response.text}")
            try_count += 1
...
# Fungsi untuk menghasilkan email dengan tambahan 5-12 huruf acak
def generate_email(base_email):
    email_parts = base_email.split('@')
    random_string = ''.join(random.choices(string.ascii_lowercase, k=random.randint(5, 12)))
...
# Fungsi untuk melakukan ping ke DNS
def ping_dns():
    hostname = "8.8.8.8"  # Ganti dengan alamat DNS yang ingin Anda ping
    try:
        result = subprocess.run(['ping', '-c', '1', hostname], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            print(f"Ping ke {hostname} berhasil. Response time: {result.stdout.strip().splitlines()[-1]}")
        else:
            print(f"Ping ke {hostname} gagal.")
    except subprocess.TimeoutExpired:
        print(f"Ping ke {hostname} timeout.")
